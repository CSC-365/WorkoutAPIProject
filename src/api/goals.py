...
def create_goal(goal: GoalJson):
    ...
    with db.engine.begin() as conn:
        ...
        newGoalId = conn.execute(
            text("SELECT MAX(goal_id) FROM goals")).fetchone()[0]
        newWorkoutId = conn.execute(
            text("SELECT MAX(workout_id) FROM workouts")).fetchone()[0]

        if goal.type_id == 0:   # weight type --> run
            conn.execute(goals.insert().values(goal_id=0 if newGoalId is None else newGoalId + 1, type_id=goal.type_id,
                         user_id=goal.user_id, target_weight=goal.target_weight, workout_id=newWorkoutId))
            user = conn.execute(
            ...
            daily_caloric_deficit = 500 * \
                (user.starting_lbs - goal.target_weight)
